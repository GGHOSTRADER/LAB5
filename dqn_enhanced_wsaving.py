...
# Flags: --double-dqn, --per, --n-step N, --huber-loss, --dueling-dqn, --reward-clip, --noisy-net, --train-frequency
...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        self.wandb_save_model = args.wandb_save_model
    ...
    def run(self, episodes=10000):
        for ep in range(episodes):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                # Train every `train_frequency` env steps
                ...
                for ms in self.milestones:
                    if self.env_count >= ms and ms not in self.saved_milestones:
                        ...
                        # Upload milestone weights to W&B as a versioned artifact
                        if self.wandb_save_model:
                            artifact = wandb.Artifact(
                                name=f"{wandb.run.name}-milestone",
                                type="model-milestone",
                                metadata={
                                    "env_step": int(self.env_count),
                                    "milestone": int(ms),
                                    "episode": int(ep),
                                },
                            )
                            artifact.add_file(path)
                            wandb.log_artifact(artifact, aliases=[f"step-{ms}"])
            ...
            if ep % 20 == 0:
                ...
                if eval_reward > self.best_reward:
                    ...
                    # Upload new best weights to W&B as a versioned artifact
                    if self.wandb_save_model:
                        artifact = wandb.Artifact(
                        ...
                        artifact.add_file(path)
                        wandb.log_artifact(artifact, aliases=["best", f"ep-{ep}"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train-frequency",          type=int, default=4,
                        help="Initiate a training cycle every N environment steps")
    ...
    parser.add_argument("--wandb-save-model", action="store_true", default=False,
                        help="Upload milestone and best-model checkpoints to W&B as artifacts")
