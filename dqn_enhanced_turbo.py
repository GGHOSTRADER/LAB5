...
class PrioritizedReplayBuffer:
    ...
    """
    def __init__(self, capacity, alpha=0.6, beta=0.4, beta_anneal_steps=250_000):
    ...
    def sample(self, batch_size):
        ...
        states, actions, rewards, next_states, dones, n_steps = zip(*batch)
        return states, actions, rewards, next_states, dones, n_steps, indices, weights.astype(np.float32)
    ...
    def update_priorities(self, indices, errors):
        ...
        # Recompute max from current priorities (don't let it monotonically grow)
...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        self.soft_target_tau         = getattr(args, 'soft_target_tau', 0.0)
    ...
    def _get_n_step_transition(self):
        ...
        # Store n_used as part of the transition so the loss uses the correct gamma exponent
        return (init_state, init_action, n_step_return, final_next_state, final_done, n_used)
    ...
    def run(self, episodes=10000):
        for ep in range(episodes):
            ...
            # Discard partial n-step transitions at episode end
            # (they would have inconsistent bootstrap targets across episodes)
            self.n_step_buffer.clear()
    ...
    def train(self):
        ...
        states, actions, rewards, next_states, dones, n_steps, indices, weights = self.memory.sample(self.batch_size)
        ...
        n_steps     = torch.tensor(n_steps, dtype=torch.float32).to(self.device)
        ...
        with torch.no_grad():
            ...
            # Use per-sample gamma exponent so partial n-step transitions are correct
            gamma_powers = self.gamma ** n_steps
            target = rewards + gamma_powers * next_q * (1.0 - dones)
        ...
        if self.soft_target_tau > 0.0:
            # Polyak averaging every gradient step
            with torch.no_grad():
                for tp, p in zip(self.target_net.parameters(), self.q_net.parameters()):
                    tp.data.mul_(1.0 - self.soft_target_tau).add_(self.soft_target_tau * p.data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--replay-start-size",        type=int, default=1_600)
    ...
    parser.add_argument("--soft-target-tau", type=float, default=0.0,
                        help="If > 0, do Polyak averaging every gradient step with this tau "
                             "(e.g. 0.005). When > 0, target_update_frequency is ignored.")
