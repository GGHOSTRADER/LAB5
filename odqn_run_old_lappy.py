...
class DQNAgent:
    ...
    def _obs_to_state(self, obs, reset=False):
        if self.is_atari:
            return (
                self.preprocessor.reset(obs) if reset else self.preprocessor.step(obs)
            )
        ...
        for ep in range(episodes):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                if self.env_count % 1000 == 0:
                    print(
                        f"[Collect] Ep:{ep} SC:{self.env_count} UC:{self.train_count} Eps:{self.epsilon:.4f}"
                    )
                    wandb.log(
                        {
            ...
            print(
                f"[Eval] Ep:{ep} Reward:{total_reward} SC:{self.env_count} Eps:{self.epsilon:.4f}"
            )
            wandb.log(
                {
                    "Episode": ep,
            ...
            if ep % 30 == 0:
                ...
                print(
                    f"[TrueEval] Ep:{ep} EvalReward:{eval_reward:.2f} SC:{self.env_count}"
                )
                wandb.log(
                    {
                        "Env Step Count": self.env_count,
    ...
    def train(self):
        ...
        if self.use_huber:
            ...
            loss = (
                weights * F.smooth_l1_loss(q_values, target.detach(), reduction="none")
            ).mean()
        else:
            # Weighted MSE
        ...
        if self.train_count % 1000 == 0:
            print(
                f"[Train #{self.train_count}] Loss:{loss.item():.4f} Q mean:{q_values.mean().item():.3f}"
            )
            wandb.log(
                {
                    "Loss": loss.item(),
